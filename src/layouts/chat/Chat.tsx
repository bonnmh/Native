...
const MessageItem: FC<{onPress: () => void; item: number}> = ({
  onPress,
  item,
}) => {
    ...
    <Button onPress={onPress}>
      ...
      >
        ...
        <Block block paddingHorizontal={spacing.normal}>
          ...
          <Text preset="textSmall" numberOfLines={2}>
...
const Chat = () => {
  ...
  const _onItem = (): void => {
    navigate(APP_SCREEN.CHAT_DETAIL);
  };

  const _renderItem = useMemo(
    () =>
      ({item, index}: {item: number; index: number}): React.ReactElement => {
  ...
  return (
    <Screen unsafe backgroundColor="white">
      <Block block paddingBottom={SpacingDefault.medium} marginTop={insets.top}>
        <Block
          alignItems="center"
          borderBottomWidth={StyleSheet.hairlineWidth}
          borderBottomColor="gray"
